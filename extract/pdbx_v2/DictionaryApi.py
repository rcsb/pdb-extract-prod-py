...
from extract.pdbx_v2.PdbxContainers    import CifName
from extract.pdbx_v2.DataCategory      import DataCategory
from extract.pdbx_v2.Method        import MethodDefinition,MethodReference

class DictionaryApi(object):
    def __init__(self,containerList,consolidate=True,expandItemLinked=False,verbose=False,log=sys.stderr):
    ...
    def __makeCategoryGroupIndex(self):
        ...
        for catName in catNameList:
            ...
            #self.__lfh.write("Category %s group list %r\n" % (catName,groupNameList))
    ...
    def getCategoryGroupCategories(self,groupName):
        try:
            ...
            self.__lfh.write("DictionaryApi.getCategoryGroupCategories failed for group %s \n" % groupName)
        ...
        for dObj in self.__containerList:
            ...
            if dc is not None:
                ...
                for row in dc.getRowList():
                    pVal=row[idxP]
                    cVal=row[idxC]
                    #self.__lfh.write("%s found parent %s child %s \n" % (dObj.getName(),pVal,cVal))
                    if pVal == cVal:
                        continue
        ...
        if not followAncestors:
            ...
            if ((v0 is None) or (len(v0) < 1) or (v0 in ['.','?'])):
                ...
                if ((pItem is not None) and (len(pItem) > 0) and (pItem != CifName.itemName(category,attribute))):
                    if self.__debug:
                        self.__lfh.write("DictionaryApi.__get() Reassigning enum code %s  category %s attribute %s to parent %r\n" % 
    ...
    def __addItemLinkToDef(self,dObj,parent_name,child_name):
        ...
        if dObj.exists('item_linked'):
            ...
            if not iFound: 
                ...
                if (self.__debug):
                    self.__lfh.write("+DictionaryApi.__addItemLinkToDef() appending item link in category %s\n" % dObj.getName())
            return True
        else:
            ...
            dObj.append(cObj)
            if (self.__debug):
                self.__lfh.write("+DictionaryApi.__addItemLinkToDef() created new item link in category %s\n" % dObj.getName())
            return True
                        ...
                        for row in cObj.getRowList():
                            ...
                            if childItemName != itemName:
                                ...
                                if childItemName in fullIndex:
                                    ...
                                    self.__lfh.write("+DictionaryApi.__expandLoopedDefinitions() missing child item %s\n" % (childItemName))
                            else:
                ...
                if (ob.getType() == 'definition') and ob.exists('item_linked'):
                    ...
                    if cObj.getRowCount() > 0:
                        ...
                        for row in cObj.getRowList():
                            ...
                            childItemName= row[idxC]
                            if childItemName != itemName:
                                self.__lfh.write("+DictionaryApi.__expandLoopedDefinitions() item name %s child item %s\n" % (itemName,childItemName))
                                iChanges+=1
                                if childItemName in fullIndex:
                                    #
                                    # Add this p/c link to the child definition -
                                    ...
                                    # error missing child definition object.
                                    self.__lfh.write("+DictionaryApi.__expandLoopedDefinitions() missing definition child item %s\n" % (childItemName))
                        if iChanges > 0:
                            ob.remove('item_linked')
    ...
    def __consolidateDefinitions(self):
                ...
                for d in dObjL[1:]:
                    ...
                    for nm in xList:
                        if nm not in dObjL[0].getObjNameList():
                            if (self.__debug):
                                self.__lfh.write("+DictionaryApi().__consolidateDefinitions() adding %s to %s\n" % (nm,name))
                            catObj=d.getObj(nm)
        ...
        for nm in nList:
            if nm in fullIndex:
                ...
                self.__lfh.write("+DictionaryApi().__consolidate() missing object name %s\n" % nm)
	# update lists 
    ...
    def __getDataSections(self):
        ...
        for ob in self.__containerList:
            ...
            if (ob.getType() == 'data'):
                if (self.__debug):
                    self.__lfh.write("+DictionaryApi().__getDataSections() adding data container name %s  type  %s  \n" % (ob.getName(),ob.getType()))
                ...
                if tl is not None:
                    ...
                    self.__itemLinkedGroupDict={}
                    for row in tl.getRowList():
                        if tl.hasAttribute('category_id') and tl.hasAttribute('link_group_id') and tl.hasAttribute('label') and tl.hasAttribute('context') and tl.hasAttribute('condition_id'):
                ...
                if tl is not None:
                    ...
                    self.__itemLinkedGroupItemDict={}
                    for row in tl.getRowList():
                        if tl.hasAttribute('child_category_id') and tl.hasAttribute('link_group_id') and tl.hasAttribute('child_name') and tl.hasAttribute('parent_name') and tl.hasAttribute('parent_category_id'):
                            ...
                            self.__itemLinkedGroupItemDict[(child_category_id,link_group_id ) ].append( ( row[tl.getIndex('child_name')], row[tl.getIndex('parent_name')],row[tl.getIndex('parent_category_id')] ))
